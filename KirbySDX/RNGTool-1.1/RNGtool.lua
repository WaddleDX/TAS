...
local DEFAULT_RNGTABLE_NEXTSTEPS = 30
...
function RNGget()
	if memory.usememorydomain(MEMORY_DONAIN_NAME) == false then
			error("error: The memory domain named " .. MEMORY_DONAIN_NAME .. " is not supported by this core.")
	end
...
-- Reset sub RNG count
function resetCount()
	resetCnt = rnglist["RNGtoCnt"][curRnd + 1]
end
...
---------------------
-- View RNG Count
function RNGcountView(Count, subCount)
	local RNGCntOffsetX = 8 * client.getwindowsize()
	local RNGCntOffsetY = 8 * client.getwindowsize()
	if ID_FROM_POWERON then
		Count = Count - 15687
		Count = wrapValueInRange(Count, 0, 65533)
	end
	gui.text(RNGCntOffsetX, RNGCntOffsetY, "ID: " .. Count, "cyan", "topright")
	gui.text(RNGCntOffsetX, RNGCntOffsetY + 16, "Sub: " .. subCount, "white", "topright")
end

-- View RNG module
function RNGview(view_prev, view_next, mark_ref, Rcnt)
	local txt_ofs_x = 8 * client.getwindowsize()
	local txt_ofs_y = 8 * client.getwindowsize() + 48
	local txt_h = 16
...
-- RNG gui module
function drawRNG(val, x, wid, str)
	-- Color change to match marking
	local color = "white"
	local backcolor = 0x00000000
	if str == "cur" then
		color = "cyan"
		backcolor = 0x6000a0e9
	elseif str == "mark" then
		color = "yellow"
		backcolor = 0x60ffff00
	end

	-- Cursor placement
	gui.drawBox(60 + x, 284, 60 + x + wid - 1, 315, backcolor, backcolor)

	-- Bar placement
	local hgt = val / 8
	gui.drawBox(60 + x, 315 - hgt, 60 + x + wid - 1, 315, color, color)
...
function RNGgui(gui_prev, gui_next, figw, mark_ref, Rcnt)
	...
	for i = -gui_prev, gui_next do
		...
		drawRNG(val, (figw + 1) * index, figw, markTable[i + mark_ref])
		index = index + 1
	end
end
...
if file then
	SQL.opendatabase(dbfile)
else
	error("error: " .. dbfile .. " does not exist in the current directory.")
end
...
--- create form
forms.destroyall()
form = forms.newform(400, 270, "RNG Tool")
...
local IDview_chkbox1 = forms.checkbox(form, "RNG ID")
forms.setsize(IDview_chkbox1, 80, 16)
forms.setlocation(IDview_chkbox1, IDview_x, IDview_y)
forms.setproperty(IDview_chkbox1, "Checked", RNGTABLE_CHECKED)

local IDview_button1 = forms.button(form, "Reset SubID", resetCount) 
forms.setsize(IDview_button1, 80, 24)
forms.setlocation(IDview_button1, IDview_x + 80, IDview_y - 4)
...
local mnp_chkbox1 = forms.checkbox(form, "RNG manipulator")
...
while true do
	...
	local subCount = wrapValueInRange(Rcnt - resetCnt, -32766, 32767)
	...
	-- show RNGCount
	if forms.ischecked(IDview_chkbox1) then
		RNGcountView(Rcnt, subCount)
	end
